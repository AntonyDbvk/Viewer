...
namespace Viewer.Render.DrawStrategy.Base
{
    ...
    {
        protected void DrawFaces(Graphics g, Vertex[] vertices, Face[] faces, Brush faceBrush, Camera camera, Size clientSize, bool isOrthogonal)
        {
            PointF[] projectedPoints = new PointF[vertices.Length];
            for (int i = 0; i < vertices.Length; i++)
            {
                projectedPoints[i] = camera.Project(vertices[i].X, vertices[i].Y, vertices[i].Z, clientSize, isOrthogonal);
            }
